...
$SaveDir = "checkpoints_official_12to1_numeric_dual_safeneg"
$Seeds = @(42, 123, 2024)
$Leads = @(1, 2, 3, 4)
  ...
  foreach ($Seed in $Seeds) {
    ...
    Invoke-IfMissing "$SaveDir\$RunName\best_model.pt" {
      & $Python train_typhoformerpp.py `
        ...
        --lambda-smooth 0.02 --lambda-direct-aux 0.25 --lambda-cv-aux 0.25 --lambda-gate-prior 0.03 `
...
& $Python summarize_official_12to1_numeric_dual_safeneg.py `
  --checkpoints-dir $SaveDir `
  --output-json official_12to1_numeric_dual_safeneg_summary.json `
  --output-md official_12to1_numeric_dual_safeneg_summary.md
